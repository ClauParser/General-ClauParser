...
namespace clau_parser {
	...
	private:
		...
		inline static const BomInfo bomInfo[1] = { // iniline + static c++17 ~
	...
	public:
		...
		static Token Get(int64_t position, char ch) {
		...
		// todo - rename.
		static Token Get(int64_t position, int64_t length, char ch) {
			Token token;
		...
		static Token Get2(int64_t position, int64_t length) {
			Token token;
		...
		static int64_t GetIdx(Token x) {
			return x.start;
		}
		static int64_t GetLength(Token x) {
			return x.len;
		}
		static int64_t GetType(Token x) { //to enum or enum class?
			return x.type;
		}
		static bool IsToken2(Token x) {
			return x.isToken2;
		}
	...
	private:
				...
				for (int64_t i = 0; i < length; ++i) {
					...
					case '\"':
						...
						{//
							...
							token_arr[num + token_arr_count].isToken2 = true;
			...
			Token*& _token_arr, size_t& _token_arr_size, bool use_simd)
		{
			...
			size_t qouted_start = -4;
			size_t slush_start = -5;

			for (size_t t = 0; t < thr_num; ++t) {
				for (size_t j = 0; j < token_arr_size[t]; ++j) {
					const size_t i = start[t] + j;

					//std::cout << tokens[i] << "\n";
					//Utility::PrintToken(text, tokens[i]);
					const size_t idx = Utility::GetIdx(tokens[i]);
					...
					if (isToken2) {
						...
						if (0 == state && '\"' == ch) {
							...
							if (0 == state && LoadDataOption::LineComment == ch) {
								...
								{
									int64_t before_idx = Utility::GetIdx(tokens[qouted_start]);
									int64_t len = Utility::GetIdx(tokens[i]) - before_idx + 1;

									tokens[real_token_arr_count] = Utility::Get(before_idx, len, text[before_idx]);
	...
	private:
		static Token check_syntax_error1(Token str, int* err) {
	...
	private:
		static bool __LoadData(const char* buffer, const Token* token_arr, int64_t token_arr_len, UserType* _global,
			int start_state, int last_state, UserType** next, int* err)
		{

			std::vector<Token> varVec;
			std::vector<Token> valVec;
			...
			int64_t count = 0;
			const Token* x = token_arr;
			const Token* x_next = x;
			...
			if (state != last_state) {
				*err = -2;
				throw std::string("error final state is not last_state!  : ");
			}
		...
		static int64_t FindDivisionPlace(const char* buffer, const Token* token_arr, int64_t start, int64_t last)
		{
